...
const PrizeItem: React.FC<IProps> = (props) => {
	...
	const getTotalPrize = (item: { [key: string]: any }) => (item.prizeInfoVoList || []).reduce((total, currentValue) => {
		return total + +currentValue.price;
	}, 0);

	const lotteryTitles = (item: { [key: string]: any }) => (item.prizeInfoVoList || [])
		.map((value, index) => {
			// 万人抽奖不需要后缀
	...
	return (
		<div className='prize-item' onClick={onCardClick}>
					...
					{serviceFavoriteVo?.serviceInfoVo?.serviceName ? (
						<>
							...
							<span className='prize-item__dad-name'>
								【
								<img className='prize-item__dad-avatar' src={processImageUrl(serviceFavoriteVo?.serviceInfoVo?.serviceIcon, '56x56xz')} />
								{serviceFavoriteVo?.serviceInfoVo?.serviceName}
								】
							</span>
							提供
			...
			<div className='prize-item__main'>
				<img className='prize-item__main-left' src={'https://laiy-offline-oss.laiytech.com/ipsponsorprod/f-V3cVMZfHqm5WyzR5U4sg.jpg?zoom=400x400xz'} alt='' />
				<div className='prize-item__main-right'>
					<div className='text-ellipsis prize-item__main-title'>{lotteryTitles(item)}</div>
					{item.cornerMarker ? (
						<div className='prize-item__main-tag'>
							{item.cornerMarker}11
						</div>
					) : null}
					<div className='text-ellipsis prize-item__main-prise'>
						<div className='prize-item__main-price'>奖品价值¥
							{item.priceDesc || getTotalPrize(item)}</div>
					</div>
					{item.myLotteryNum && +item.myLotteryNum > 0 ? (
						<img className='prize-item__main-btn' src={'https://sl-online-oss.shulidata.com/resource/lottery_everyday/v2_71_0/lotteryed_btn.png'} alt='' />
					) : (
						<img className='prize-item__main-btn' src={'https://sl-online-oss.shulidata.com/resource/lottery_everyday/v2_71_0/lottery_btn.png'} alt='' />
					)}
				</div>
			</div>
		</div>
